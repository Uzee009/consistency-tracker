...
class DatabaseService {
  ...
  // --- Development/Dummy Data Seed ---
  Future<void> seedData() async {
    final db = await database;
    
    // 1. Create Daily Tasks
    var dailyTasks = [
      Task(id: 101, name: 'Morning Meditation', type: TaskType.daily, durationDays: 30, isPerpetual: true, createdAt: DateTime(2026, 1, 1), isActive: true),
      Task(id: 102, name: 'Read 20 Pages', type: TaskType.daily, durationDays: 30, isPerpetual: true, createdAt: DateTime(2026, 1, 1), isActive: true),
      Task(id: 103, name: 'Workout', type: TaskType.daily, durationDays: 30, isPerpetual: true, createdAt: DateTime(2026, 1, 1), isActive: true),
    ];

    for (var task in dailyTasks) {
      await db.insert(tasksTable, task.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
    }

    // 2. Clear Feb records
    await db.delete(dayRecordsTable, where: "date LIKE '2026-02-%'");

    // 3. Generate records for Feb 1 to Feb 25
    for (int day = 1; day <= 25; day++) {
      String dateStr = '2026-02-${day.toString().padLeft(2, '0')}';
      
      List<int> completedIds = [];
      List<int> skippedIds = [];
      bool cheatUsed = (day == 5 || day == 15 || day == 22);
      
      if (!cheatUsed) {
        // Simple logic for completion variety
        if (day % 2 == 0) {
          completedIds = [101, 102, 103]; // Perfect day
        } else if (day % 3 == 0) {
          completedIds = [101];
          skippedIds = [102]; // Partial + Skip
        } else {
          completedIds = [101, 103]; // Partial
        }

        // Add a temp task bonus on some days for "Star" status
        if (day % 4 == 0) {
          int tempId = 200 + day;
          await db.insert(tasksTable, Task(id: tempId, name: 'Fix Bug $day', type: TaskType.temporary, durationDays: 1, createdAt: DateTime(2026, 2, day), isActive: true).toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
          completedIds.add(tempId);
        }
      }

      // We'll let the ScoringService handle the actual calculation when loaded, 
      // but for seeding we need to satisfy non-null constraints
      await db.insert(dayRecordsTable, {
        'date': dateStr,
        'completed_task_ids': completedIds.join(','),
        'skipped_task_ids': skippedIds.join(','),
        'cheat_used': cheatUsed ? 1 : 0,
        'completion_score': 0.0, 
        'visual_state': cheatUsed ? 'cheat' : 'level1'
      });
    }
  }
}
