...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final DashboardLayoutController _layoutController =
  ...
  Future<void> _onUpdateAvailableChanged() async {
    if (!mounted) return;
    final info = UpdateService.instance.available.value;
    if (info != null) {
      final isDismissed =
          await UpdateService.instance.isDismissed(info.version);
      if (mounted) {
        setState(
            () => _dismissedUpdateVersion = isDismissed ? info.version : null);
      }
    } else {
      setState(() => _dismissedUpdateVersion = null);
    }
  }
  ...
  Widget _buildGlobalHeader(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(
          horizontal: 24, vertical: 20), // Increased vertical space
        ...
        children: [
          ...
          Expanded(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text('CONSISTENCY TRACKER',
                    style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.w900,
                        letterSpacing: 4)),
                const SizedBox(height: 4),
                Text('$dayName, $dateStr'.toUpperCase(),
                    style: TextStyle(
                        fontSize: 9,
          ...
          Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (_dataController.isTimerRunning ||
                  _dataController.timerSecondsRemaining <
                      _dataController
                          .timerDurations[_dataController.timerMode]!) ...[
                _buildMiniTimer(context),
                const SizedBox(
                    width: 16), // Margin between timer and refresh/user menu
              ],
  ...
  Widget _buildInLayoutNavBar(bool isDark) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _buildNavTab('DASHBOARD', Icons.dashboard_rounded,
              _activeTabIndex == 0, () => setState(() => _activeTabIndex = 0)),
          _buildNavTab('EXPLORER', Icons.explore_rounded, _activeTabIndex == 1,
              () => setState(() => _activeTabIndex = 1)),
          _buildNavTab('PROFILE', Icons.person_rounded, _activeTabIndex == 2,
