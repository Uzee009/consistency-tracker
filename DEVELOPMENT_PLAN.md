...
*   **Step 13: Implement Cross-Device Sync (PocketBase, Local-First).**
    ...
    *   **Build Order (each phase shippable):**
        *   **Phase 0 — Local refactor (no network). ✅ COMPLETE (2026-05-26).** Migrated task identity from auto-increment `int id` to client-generated UUID string `sid`; split `day_records.completed_task_ids` into `task_status` table (one row per completion/skip) + `day_meta` table (per-day single-value state); completion_score/visual_state now derived locally; added sync columns (`updated_at`, `deleted`, `dirty`) to tasks, task_status, day_meta. Database v7→v8 migration automatic on first launch. Full-stack change (models, database, services, controllers, widgets). No user-visible changes — dashboard, analytics, all features work identically. Tested on Linux with dev database.
